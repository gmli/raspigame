...
class PexParticlesLite(Points):
  def __init__(self, pex_file, emission_rate=10, scale=1.0, new_batch=0.1,
                                          hardness=2.0, **kwargs):
    ...
    super(PexParticlesLite, self).__init__(vertices=np.zeros((self.maxParticles, 3), dtype='float32'), 
    ...
    shader = Shader('shaders/mat_pointsprite')
  ...
  def update(self):
    ...
    b[self.arr[:,3] <= 0.0, 6] = 0.0 # make alpha 0 for dead particles
